...
class RoomController extends ApiActiveController
{
    ...
    public function actionCreate($lang)
    {
        ...
        if(!is_array($result)){
            return $this->response(1, _e('Room successfully created.'), $model, null, ResponseStatus::CREATED);
        }else{
    ...
    public function actionUpdate($lang, $id)
    {
        ...
        if(!is_array($result)){
            return $this->response(1, _e('Room successfully updated.'), $model, null, ResponseStatus::OK);
        }else{
            return $this->response(0, _e('There is an error occurred while processing.'), null, $result, ResponseStatus::UPROCESSABLE_ENTITY);
        }
    }
